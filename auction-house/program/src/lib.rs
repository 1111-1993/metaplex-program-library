...
pub mod auction_house {
    ...
    /// Create a private buy bid by creating a `buyer_trade_state` account and an `escrow_payment` account and funding the escrow with the necessary GTH or SPL token amount.
    pub fn buy<'info>(
    ...
    /// Create a public buy bid by creating a `public_buyer_trade_state` account and an `escrow_payment` account and funding the escrow with the necessary GTH or SPL token amount.
    ...
    pub fn close_escrow_account<'info>(
        ...
        invoke_signed(
            &system_instruction::transfer(
                ...
                ctx.accounts.escrow_payment_account.weis(),
            ),
            &[
...
pub struct CreateAuctionHouse<'info> {
    /// Treasury mint account, either native GTH mint or a SPL token mint.
    pub treasury_mint: Account<'info, Mint>,

    /// Key paying GTH fees for setting up the Auction House.
    #[account(mut)]
    ...
    /// CHECK: User can use whatever they want for intialization.
    /// GTH or SPL token account to receive Auction House fees. If treasury mint is native this will be the same as the `treasury_withdrawl_destination_owner`.
    #[account(mut)]
